...
# ── Measured data (through holder) ───────────────────────────────────────
_meas = np.array([
    [10,          0.035310753,  0.003780446],
    [7.943145752, 0.035542767,  0.004498564],
    [6.309448242, 0.036131378,  0.005358283],
    [5.01159668,  0.036125686,  0.006124746],
    [3.980743408, 0.036565601,  0.007402311],
    [3.162109375, 0.037377838,  0.008774717],
    [2.511749268, 0.038277033,  0.011030777],
    [1.995239258, 0.039308829,  0.013284014],
    [1.584655762, 0.040334461,  0.015910647],
    [1.25881958,  0.04212493,   0.018925738],
    [0.999755859, 0.044844257,  0.022281353],
    [0.794250488, 0.048403913,  0.026088321],
    [0.630767822, 0.052645948,  0.030617366],
    [0.501098633, 0.058681849,  0.032949235],
    [0.397979736, 0.065769932,  0.036607205],
    [0.316162109, 0.073815928,  0.038563228],
    [0.250976563, 0.080689527,  0.039356409],
    [0.199523926, 0.089074906,  0.037907794],
    [0.15838623,  0.096436013,  0.035678454],
    [0.12588501,  0.102372822,  0.033477382],
    [0.09999,     0.108873613,  0.030135885],
    [0.079439972, 0.112376224,  0.027461742],
    [0.063101413, 0.115377342,  0.024434176],
    [0.050123234, 0.118335978,  0.022420025],
    [0.039814698, 0.119012276,  0.020404185],
    [0.031625623, 0.121982504,  0.020178247],
    [0.025121125, 0.121792897,  0.019329331],
    [0.019954618, 0.124867789,  0.020487372],
    [0.015850358, 0.12735527,   0.01896543 ],
    [0.012590513, 0.129453721,  0.022077421],
    [0.01,        0.130896748,  0.024543966],
])

def _interp_complex(freqs_src, Z_src, freqs_target):
    log_src = np.log10(freqs_src)
    log_tgt = np.log10(freqs_target)
    return (np.interp(log_tgt, log_src, Z_src.real)
          + 1j * np.interp(log_tgt, log_src, Z_src.imag))

# Parse
_f_ref  = _ref[:, 0];  _Z_ref  = _ref[:, 1]  + 1j * _ref[:, 2]
_f_meas = _meas[:, 0]; _Z_meas = _meas[:, 1] + 1j * _meas[:, 2]

# Sort ascending (interp requires monotone input)
_idx_ref  = np.argsort(_f_ref);  _f_ref  = _f_ref[_idx_ref];  _Z_ref  = _Z_ref[_idx_ref]
_idx_meas = np.argsort(_f_meas); _f_meas = _f_meas[_idx_meas]; _Z_meas = _Z_meas[_idx_meas]

# Common grid = measured frequencies clipped to overlapping range
_f_min = max(_f_ref.min(), _f_meas.min())   # 0.01  Hz
_f_max = min(_f_ref.max(), _f_meas.max())   # 10    Hz

_mask          = (_f_meas >= _f_min) & (_f_meas <= _f_max)
HOLDER_FREQS   = _f_meas[_mask]                              # Hz
HOLDER_Z       = (_Z_meas[_mask]
                  - _interp_complex(_f_ref, _Z_ref, HOLDER_FREQS))
